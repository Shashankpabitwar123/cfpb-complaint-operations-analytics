...
## 1. Set up Snowflake once

1. Create or sign in to a Snowflake account/trial.
2. In a worksheet as an appropriately privileged role, run `sql/snowflake/01_setup.sql`.
3. Grant the role to your Snowflake user (replace the placeholder locally):

   ```sql
   GRANT ROLE CFPB_PORTFOLIO_ROLE TO USER <YOUR_SNOWFLAKE_USERNAME>;
   ```

4. Copy `.env.example` to `.env` and enter the account locator and username. Keep `SNOWFLAKE_AUTHENTICATOR=externalbrowser` unless your organization requires another approved sign-in method.
...
If an upload is interrupted, resolve the error and intentionally rerun with `--replace`; do not assume a partially loaded table is valid.
